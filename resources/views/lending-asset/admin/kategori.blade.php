...
<x-lend-layout-template>
    <div>
        <div class="me-7 mt-1">
            @php              
                ...
                $description = 'Halaman ini berisi daftar Kategori yang terdaftar yang dapat Anda kelola.';
                $add = 'Tambah Kategori Baru';
                $columns = [
                    ['key' => 'id', 'title' => 'ID'],
                    ['key' => 'name_category', 'title' => 'Nama Kategori'],
                    ['key' => 'description_category', 'title' => 'Deskripsi']
                ];              
            @endphp              
            ...
            <div id="table"
                ...
                <div class="relative mx-4 mt-4 overflow-hidden text-gray-700 bg-white rounded-none">
                    ...
                    <div class="ms-5 me-5 flex p-6 px-0 -mt-5 overflow-hidden">
                        <div class="w-full rounded-xl overflow-hidden border border-blue-gray-100">
                            <table id="usertable" class="w-full mt-4 text-left table-auto">
                                ...
                                <tbody>
                                    @forelse ($item_categories as $itemCategory)                               <tr>
                                            <td
                                                class="p-4 border-b border-blue-gray-100 text-sm text-blue-gray-900 text-center">
                                                {{ $item_categories->firstItem() + $loop->index }}
                                            </td>
                                            ...
                                            <td class="border-b border-blue-gray-100">
                                                <div class="flex items-center gap-3 justify-center">
                                                    ...
                                                    <button type="button" class="text-blue-500 flex items-center gap-2"
                                                        onclick="openEditModal('{{ $itemCategory->id }}', '{{ $itemCategory->name_category}}', '{{ $itemCategory->description_category }}')">
                                                    ...
                                                    {{-- Tombol Hapus --}}
                                                    <form action="{{ route('kategori.destroy', $itemCategory->id) }}" method="POST" class="inline">  
                                                        @csrf  
                                                        @method('DELETE')  
                                                        <button type="button" class="text-red-500 flex items-center gap-1 delete-button" data-id="{{ $itemCategory->id }}" data-name_category="{{ $itemCategory->name_category }}" onclick="showDeleteConfirmation(this)">  
                    ...
                    <div id="paginasi" class="flex items-center justify-between p-4 mt-1 border-t border-blue-gray-50">
                        <p class="block font-sans text-sm font-normal leading-normal text-blue-gray-900">
                            Total Data: {{ $item_categories->total() }} | Page {{ $item_categories->currentPage() }} of
                            {{ $item_categories->lastPage() }}
                        </p>
                        <div class="flex gap-2 me-2">
                            @if($item_categories->onFirstPage())
                                <span
                            ...
                            @else
                                <a href="{{ $item_categories->previousPageUrl() . (request('search') ? '&search=' . urlencode(request('search')) : '') }}"
                            ...
                            @endif
                            @if($item_categories->hasMorePages())
                                <a href="{{ $item_categories->nextPageUrl() . (request('search') ? '&search=' . urlencode(request('search')) : '') }}"
                                    class="select-none rounded-lg border border-gray-900 py-2 px-4 text-center font-sans text-xs font-bold uppercase text-gray-900 transition-all hover:opacity-75">
                                    Next
                                </a>
                            @else
                 ...
                 <div id="addModal"
                    ...
                    <div class="bg-white rounded-lg p-6 w-1/3">
                        ...
                        <form id="addForm" action="{{ route('kategori.store') }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label for="addNamaKategory" class="block text-sm font-medium text-gray-700 mb-2">Nama Kategori</label>
                                <input type="text" name="name_category" id="addNamaKategory"
                                    class="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-md outline-blue-600"
                                    placeholder="Tambahkan Nama Kategori Baru" required>
                            </div>
                            <div class="mb-4">
                                <label for="addDeskripsiKategori" class="block text-sm font-medium text-gray-700 mb-2">Deskripsi Kategorii</label>
                                <input type="text" name="description_category" id="addDeskripsiKategori"
                                    class="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-md outline-blue-600"
                ...
                <div id="editModal"
                    ...
                    <div class="bg-white rounded-lg p-6 w-1/3">
                        ...
                        <form id="editForm" action="{{ route('kategori.update', '') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')                             
                            
                            <input type="hidden" name="id" id="editKategoriId"> <!-- Input tersembunyi untuk ID -->

                            <div class="mb-4">
                                <label for="editNamaKategori" class="block text-sm font-medium text-gray-700 mb-2">Nama Kategori</label>
                                <input type="text" name="name_category" id="editNamaKategori"
                                    class="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-md outline-blue-600"
                                    placeholder="Tambahkan Nama Kategori Terbaru" required>
                            </div>

                            <div class="mb-4">
                                <label for="editDeskripsiKategori"
                                    class="block text-sm font-medium text-gray-700 mb-2">Deskripsi Kategori</label>
                                <input type="text" name="description_category" id="editDeskripsiKategori"
                                    class="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-md outline-blue-600"
                                    placeholder="Tambahkan Deskripsi Kategori" required>
                            </div>
    ...
    <script>
        ...
        function openEditModal(id, name_category, description_category) {
            document.getElementById('editKategoriId').value = id; // Set ID pengguna  
            document.getElementById('editNamaKategori').value = name_category; // Set value Username  
            document.getElementById('editDeskripsiKategori').value = description_category; // Set value Email  

            // Set action form edit  
            document.getElementById('editForm').action = "{{ route('kategori.update', '') }}/" + id; // Set action form edit  
        ...
        function showDeleteConfirmation(button) {  
            ...
            const namaKategori = button.dataset.name_category; // Get user name      
            document.getElementById('deleteItemName').textContent = namaKategori; // Set user name in modal      
