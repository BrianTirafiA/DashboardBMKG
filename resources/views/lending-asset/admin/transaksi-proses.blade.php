...
<head>
    ...
    <title>Daftar Pemohonanan Diterima - Admin</title>
</head>
...
<x-lend-layout-template>
    <div>
        <div class="me-7 mt-1">
            @php      
                $title = 'Daftar Permohonan diterima';
                $description = 'Halaman ini berisi daftar Permohonan diterima yang dapat Anda kelola.';
                $columns = [
                    ['key' => 'id', 'title' => 'ID'],
                    ...
                    <div class="flex items-center justify-between gap-8 mb-4 ms-5 mt-2">
                        ...
                        <div class="flex flex-col gap-2 shrink-0 me-5 sm:flex-row">
                            <div class="w-full md:w-72">
                                <form action="{{ route('proses.index') }}" method="GET"
    ...
    function updateCountdown(returnedDate, elementId) {  
        ...
        if (daysRemaining > 0) {  
            countdownElement.innerText = `${daysRemaining} hari tersisa`;  
        } else {  
            countdownElement.innerText = 'Sudah lewat';  
        }  
    }  
        ...
        <div class="bg-white rounded-lg p-6 w-1/2">
            ...
            <div class="flex w-full space-x-5">
                ...
                <div class="flex-row w-full">
                    ...
                    <div class="mb-4">
                        <label for="modalApprovalDate" class="block text-sm font-medium text-gray-700 mb-2">Tanggal
                            Permohonan Disetujui</label>
                        <span id="modalApprovalDate"
                            class="block w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-md bg-gray-100">
                        </span>
                    </div>
    ...
    <script>
        ...
        function openDetailModal(id, fullname, email, nip, no_telepon, unit_kerja, loanDuration, loanReason, submissionDate, approval_date, items) {
            ...
            document.getElementById('modalSubmissionDate').textContent = formatDate(submissionDate);
            document.getElementById('modalApprovalDate').textContent = formatDate(approval_date);
    ...
    <div id="confirmationModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg p-6 w-1/3">
            ...
            <form id="acceptForm" action="{{ route('transaksi-proses.update', '') }}" method="POST">
                @csrf
                @method('PUT')                
                ...
                <div class="mb-4">
                    <label for="note" class="block text-sm font-medium text-gray-700">Pesan Konfirmasi</label>
    ...
    document.getElementById('returnedDate').value = returnedDate.toISOString().split('T')[0]; // Format YYYY-MM-DD    
     ...
     document.getElementById('userEmail').value = email; // Menambahkan email ke input hidden
  
    // Menampilkan modal    
    document.getElementById('confirmationModal').classList.remove('hidden');    
    document.getElementById('acceptForm').action = "{{ route('transaksi-proses.update', '') }}/" + id;    
